  ...
  return (
    <div style={styles.loadingScreen}>
      <div style={styles.loadingContent}>
        ...
        <div style={styles.loadingPercent}>{Math.floor(progress)}%</div>
      </div>
    </div>
  );
}
...
function YoList({ setView }) {
  const { currentUser, users } = React.useContext(AuthContext);
                ...
                <div key={yo.odomain} style={styles.yoItem}>
                  <div style={styles.yoItemHeader}>
                    <span style={styles.yoName}>
                      {getCountryFlag(yo.country)} {yo.firstName} {yo.lastName}
                    </span>
...
function ChatList({ setView }) {
  const { userChats, users } = React.useContext(AuthContext);
              ...
              userChats.map((chat) => (
                ...
                >
                  <span style={styles.chatListName}>
                    {getCountryFlag(chat.otherUser.country)} {chat.otherUser.firstName} {chat.otherUser.lastName}
                  </span>
...
function Dashboard({ setView }) {
  ...
  const handleMatch = async () => {
    ...
    if (foundMatches.length > 0) {
      playMatchSound();
      await addToYoList(foundMatches);
      // Create chat rooms for all matches
      for (const match of foundMatches) {
        await createChat(match.user.uid);
      }
    } else {
      playNoMatchSound();
    }
    ...
    <div style={styles.dashboard}>
      <div style={styles.dashHeader}>
        ...
        <div style={styles.dashUser}>
          <button style={styles.navBtn} onClick={() => setView('chats')}>💬</button>
          <button style={styles.navBtn} onClick={() => setView('yolist')}>📋</button>
          <button style={styles.navBtn} onClick={() => setView('preferences')}>⚙️</button>
          <span>{getCountryFlag(currentUser.country)} {currentUser.firstName}</span>
                    ...
                    <div key={i} style={styles.matchItem}>
                      <div style={styles.matchUserInfo}>
                        <span style={styles.matchName}>
                          {getCountryFlag(m.user.country)} {m.user.firstName} {m.user.lastName}
                        </span>
    ...
    <div style={styles.chatContainer}>
      <div style={styles.chatHeader}>
        ...
        <div style={styles.chatTitle}>
          💬 {getCountryFlag(otherUser.country)} {otherUser.firstName} {otherUser.lastName}
        </div>
      </div>
